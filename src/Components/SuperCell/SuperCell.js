...
const SuperCell = () => {
  ...
  const [switchgame, setSwitch]= useState("royal");

  // جلب البيانات من Firebase عند تحميل الصفحة
  useEffect(() => {
  const fetchGames = async () => {
    const querySnapshot = await getDocs(collection(db, switchgame)); // جلب البيانات من المجموعة المناسبة بناءً على القيمة
    const gamesList = querySnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
    setGames(gamesList); // تحديث الألعاب
  };
  fetchGames();
}, [switchgame]); // سيتم التنفيذ عندما يتغير switchgame
...
return (
    ...
    <div className="switch d-flex">
        <button className="btn btn-info text-white" onClick={() => setSwitch("royal")}>Clash Royale</button>
        <button className="btn btn-info text-white" onClick={() => setSwitch("clans")}>Clash Of Clans</button>
        <button className="btn btn-info text-white" onClick={() => setSwitch("brawl")}>Brawl Stars</button>
    </div>
